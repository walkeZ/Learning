package com.walke.testuiautomator;
 ...
 * 使用Android Device Monitor获取界面元素信息
 *
 * Ui Automator识别APP元素时报错 Error obtaining UI hierarchy
 *      https://blog.csdn.net/jis24/article/details/79391278
 * 提示"adbd cannot run as root in production builds"原因及解决方案
 *      http://www.yzmg.com/news/show-86548.html
 *
 */
...
public class AutomatorTest {
    ...
    private static final String PACKAGE_ESPRESSOTESTS = "com.walke.testespresso";
...
//        UiScrollable scrollview = new UiScrollable(new UiSelector().className(ListView.class).resourceId("android:id/list"));//模拟器的
        UiScrollable scrollview = new UiScrollable(new UiSelector().className(ListView.class).resourceId("android:id/list"));//模拟器的
        //滑动到底部
        scrollview.flingForward();
        //通过文本找到关于手机
        UiObject aboutPhone = scrollview.getChild(new UiSelector().text("关于手机"));//模拟器的
        //点击跳转到手机信息界面
        aboutPhone.click();
...
//        UiObject ibtnBack = mDevice.findObject(new UiSelector().className(ImageButton.class).description("向上导航"));
        UiObject ibtnBack = mDevice.findObject(new UiSelector().className(TextView.class).text("法律信息"));//模拟器的
        //（点击返回）  点击进入法律信息
        ibtnBack.click();
        //点击系统返回键
        mDevice.pressBack();
        ...
        mDevice.pressBack();
        //滑动到包含"提示音和通知"的地方
        scrollview.scrollTextIntoView("声音");// 系统设置中的一个选项，不同手机不同
        //通过显示的文本找到控件
        UiObject notify = scrollview.getChild(new UiSelector().text("声音"));
        //点击跳转到下一个界面
        notify.click();
        //通过显示的文本"手机铃声"找到控件
