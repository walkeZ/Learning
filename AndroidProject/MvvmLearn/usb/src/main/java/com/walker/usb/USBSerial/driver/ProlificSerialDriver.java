...
import com.walker.usb.USBSerial.util.MonotonicClock;
...
public class ProlificSerialDriver implements UsbSerialDriver {
    ...
    class ProlificSerialPort extends CommonUsbSerialPort {
        ...
        private void readStatusThreadFunction() {
            ...
            //LogUtil.d("end control line status thread " + mStopReadStatusThread + " " + (mReadStatusException == null ? "-" : mReadStatusException.getMessage()));
        }
            ...
            if ((mReadStatusThread == null) && (mReadStatusException == null)) {
                synchronized (mReadStatusThreadLock) {
                    if (mReadStatusThread == null) {
                        ...
                        //LogUtil.d("start control line status thread " + mStatus);
        ...
        public void openInt() throws IOException {
            ...
            LogUtil.d(String.format("usbVersion=%x, deviceVersion=%x, deviceClass=%d, packetSize=%d => deviceType=%s",
                ...
                synchronized (mReadStatusThreadLock) {
                    if (mReadStatusThread != null) {
                        try {
                            ...
                            LogUtil.e("An error occured while waiting for status read thread" + e.getMessage());
                        }
        ...
        private int filterBaudRate(int baudRate) {
            if ((baudRate & (3 << 29)) == (1 << 29)) {
                LogUtil.i("filterBaudRate: " + baudRate);
            ...
            LogUtil.d(String.format("baud rate=%d, effective=%d, error=%.1f%%, value=0x%08x, mantissa=%d, exponent=%d",
