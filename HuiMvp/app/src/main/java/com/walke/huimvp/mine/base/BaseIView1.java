 ...
 * 3.P-presenter：(代理者)数据处理层，所有的的数据逻辑，业务逻辑都在这里处理。然后通过接口回调通知View做出相应刷新。故需要持有IView的接口引用(对象)
 ...
 *           等等
 *        父类代码构思：
 *          1.IView：主要做业务逻辑回调通知，应该会有一些常用回调，比如显示、隐藏加载进度条。然后在实现类的父类
 *           (BaseActivity2/BaseFragment)中同一处理，这是Java面向对象的特性继承的体现。
 *          2.Presenter:业务逻辑都在这里处理。然后通过接口回调通知View做出相应刷新。故需要持有IView的接口引用(对象),
 *            因为IView有多个不同实现类(子类)应该用泛型封装。
 *          3.
 *
 *
 */

public interface BaseIView1<T extends BasePresenter1> {
